...
func (s *server) Run() error {
	...
	if s.config.DNSSEC == "" {
		...
		s.config.log.Printf("ready for queries on %s for %s [rcache %d - ttl %d], signing with %s [scache %d - ttl %d]", s.config.Domain, s.config.DnsAddr, s.config.RCache, s.config.RCacheTtl, s.config.DNSSEC, s.config.SCache)
	}
