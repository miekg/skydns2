...
func init() {
	...
	flag.StringVar(&msg.PathPrefix, "path-prefix", env("SKYDNS_PATH_PREFIX", msg.PATH_PREFIX), "backend(etcd) path prefix, default: skydns")
}
...
func loadConfig(client *etcd.Client, config *server.Config) error {
	...
	n, err := client.Get(fmt.Sprintf("%s/config", msg.Pathprefix()), false, false)
	if err != nil {
